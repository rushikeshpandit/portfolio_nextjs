  ...
  return (
    <FadeIn delay={index * 0.05}>
      <div className="card" style={{ overflow: "hidden" }}>
        ...
        >
          {/* Left: company + role */}
          <div className="exp-meta" style={{ display: "flex", alignItems: "flex-start", gap: 16, flex: 1, flexWrap: "wrap" }}>
            <div style={{ flex: 1, minWidth: 0 }}>
              <div
                className="font-display"
                style={{ fontSize: "1.0625rem", fontWeight: 700, color: "var(--text)", letterSpacing: "-0.01em", marginBottom: 2 }}
              >
                {entry.company}
              </div>
              <div style={{ fontSize: "0.875rem", color: "var(--accent)", fontWeight: 500 }}>
                {entry.designation}
              </div>
            </div>
          ...
          <div className="exp-card__body-inner">
            <ul style={{ listStyle: "none", display: "flex", flexDirection: "column", gap: 10, paddingTop: 4 }}>
              {entry.responsibilities.map((r, ri) => (
                <li
                  key={ri}
                  style={{ display: "flex", gap: 10, alignItems: "flex-start", fontSize: "0.875rem", color: "var(--text-muted)", lineHeight: 1.65 }}
                >
...
export default function Experience() {
  const workEntries = info.experience.filter((e): e is WorkEntry => e.isWork);
  const eduEntries  = info.experience.filter((e): e is EduEntry => !e.isWork);
        ...
        {eduEntries.map((entry, i) => (
          <FadeIn key={entry.degree} delay={i * 0.08}>
            <div className="card" style={{ padding: "24px 28px", flex: 1, minWidth: 260 }}>
              <div className="font-display" style={{ fontSize: "0.9375rem", fontWeight: 700, color: "var(--text)", marginBottom: 4 }}>
                {entry.degree}
              </div>
              <div style={{ fontSize: "0.8125rem", color: "var(--accent)", marginBottom: 2 }}>
                {entry.college}
              </div>
              ...
              {entry.achievements.map((a, ai) => (
                <div key={ai} style={{ fontSize: "0.8125rem", color: "var(--text-muted)", display: "flex", gap: 8, alignItems: "flex-start", marginBottom: 6 }}>
